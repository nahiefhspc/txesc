...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog):
    ...
    try:
        probe_cmd = f'ffprobe -v quiet -print_format json -show_streams "{filename}"'
        probe_output = subprocess.check_output(probe_cmd, shell=True).decode('utf-8')
        ...
        await m.reply_text(f"Error detecting resolution: {str(e)}")
        return
    ...
    try:
        # Process video with compression, skipping first 15 seconds
        ...
        if height == 480:
            display_width = int(width * (480 / height))  # Maintain aspect ratio
            display_height = 480
        elif height == 720:
            display_width = int(width * (720 / height))  # Maintain aspect ratio
            display_height = 720
