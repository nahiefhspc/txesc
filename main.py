...
async def txt_handler(bot: Client, m: Message):
                ...
                def fetch_with_retries(local_url, headers=None, max_retries=10):
                    ...
                    if token_response:
                        try:
                            ...
                            session_token = token_data.get("use_token", "raw_text98")  # Extract use_token or fallback
                            print(f"Successfully fetched session token: {session_token}")
                        except json.JSONDecodeError:
                            print("❌ Invalid JSON in token API response, using default session token.")
                    else:
                        print("❌ Failed to fetch session token, using default session token.")
                    ...
                    for quality in qualities:
                        for attempt in range(max_retries):
                            try:
                                ...
                                new_url = f"{base_path}hls/{quality}/main.m3u8" + (f"?{query_params}" if query_params else '')
    ...
    # Step 3: Headers for video metadata request
            ...
            if "jw-prod" in url:
               ...
               if end_time:
                   cmd = f'yt-dlp --concurrent-fragments 30 --download-sections "*{skip_seconds}-{end_time}" -f "{ytf}" "{url}" -o "{name}.mp4"'
               else:
